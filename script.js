...
const MILESTONE_APPROACH_RANGE = 15;
...
    STONE: { id: 3, color: '#555555', breakable: false, fallable: true, hp: Infinity },
...
const BLOCK_INFO = {};
for (const key in BLOCK_TYPES) { BLOCK_INFO[BLOCK_TYPES[key].id] = BLOCK_TYPES[key]; }
...
let lastPauseTime = 0; let nextMilestonePhysicalY = 100;
...
function startGameFlow() { currentGameState = GAME_STATE.PLAYING; updateUIVisibility(); startGame(); }
...
singlePlayerBtn.addEventListener('click', startGameFlow); retryBtn.addEventListener('click', startGameFlow);
mainMenuBtn.addEventListener('click', showLandingPage); resumeBtn.addEventListener('click', resumeGame);
pauseGameBtn.addEventListener('click', togglePause);
pauseRestartBtn.addEventListener('click', startGameFlow); pauseGraphicsSettingsBtn.addEventListener('click', showGraphicsMenuFromPause);
pauseMainMenuBtn.addEventListener('click', showLandingPage); bloomToggle.addEventListener('change', () => graphicsSettings.bloom = bloomToggle.checked);
...
const makeRow = (rowIndex) => {
    ...
    const useClustering = Math.abs(rowIndex - nextMilestonePhysicalY) > MILESTONE_APPROACH_RANGE;
    ...
    for (let x = 0; x < COLS; x++) {
        ...
        const continueVerticalCluster = useClustering && blockAbove && blockAbove.typeId !== BLOCK_TYPES.SOLID.id && blockAbove.typeId !== BLOCK_TYPES.HP.id && blockAbove.typeId !== BLOCK_TYPES.SOUL.id && Math.random() < 0.6;
        ...
        } else if (useClustering && lastTypeId && lastTypeId !== BLOCK_TYPES.SOLID.id && lastTypeId !== BLOCK_TYPES.HP.id && lastTypeId !== BLOCK_TYPES.SOUL.id && Math.random() < 0.5) {
             newRow[x] = { typeId: lastTypeId, hp: BLOCK_INFO[lastTypeId].hp };
        } else {
            if (rand < HEART_RATE && rowIndex > 5) {
                newRow[x] = { typeId: BLOCK_TYPES.HP.id };
            } else if (rand < (HEART_RATE + 0.04 + rowIndex * 0.00025) && rowIndex > SOUL_BLOCK_MIN_DEPTH) {
                newRow[x] = { typeId: BLOCK_TYPES.SOUL.id, hp: BLOCK_TYPES.SOUL.hp };
            } else {
                const blockTypeRand = Math.random();
                if (blockTypeRand < 0.45) newRow[x] = { typeId: BLOCK_TYPES.SOLID.id, hp: 1 };
    ...
    if (!newRow.some(b => b && BLOCK_INFO[b.typeId].breakable)) {
        newRow[rndInt(0, COLS - 1)] = { typeId: BLOCK_TYPES.SOLID.id, hp: 1 };
    }
    return newRow;
};

function placeIronChunk(yStart) {
    const isDeep = (depthOffset + yStart) >= 200;
    ...
    for (let r = 0; r < chunkHeight; r++) {
        ...
        if (rows[currentY]) {
            for(let c = 0; c < chunkWidth; c++) {
                setBlock(startCol + c, currentY, BLOCK_TYPES.IRON.id);
            }
        }
    }
    ...
    for (let r = 0; r < chunkHeight; r++) {
        ...
        for (let c = 0; c < COLS; c++) {
            if (getBlock(c, currentY)?.typeId !== BLOCK_TYPES.IRON.id) {
                pathColumns.push(c);
                if (getBlockInfo(c, currentY).breakable) {
                    breakablePathExists = true;
                }
            }
        }
...
  digCount = hearts = 0; depthOffset = 0; nextMilestonePhysicalY = 100;
  breaking.length = fallers.length = dust.length = 0;
  camY = 0; msState = null; updateHUD();
}
function startGame() { resetWorld(); }

const ensureRow = (rowIndex) => {
    while (rowIndex >= rows.length) {
        let newRowInternalIndex = rows.length; rows.push(makeRow(newRowInternalIndex));
        if (newRowInternalIndex >= IRON_CHUNK_MIN_DEPTH &&
            newRowInternalIndex > lastIronChunkEndRow + IRON_CHUNK_MIN_GAP_BETWEEN &&
            (newRowInternalIndex % IRON_CHUNK_INTERVAL === 0 || (newRowInternalIndex % IRON_CHUNK_INTERVAL > 0 && Math.random() < 0.05))) {
...
function findConnectedBlocks(startX, startY) {
    ...
    if (!startBlock || !BLOCK_INFO[startBlock.typeId].breakable || startBlock.typeId === BLOCK_TYPES.SOUL.id || startBlock.typeId === BLOCK_TYPES.HP.id) {
        return [];
    }
...
function handleInput(ts) {
    ...
    if (targetBlockInfo.breakable) {
        ...
        if (fallerIndex > -1) {
            fallers.splice(fallerIndex, 1);
        }

        if (targetBlockInfo.id === BLOCK_TYPES.SOUL.id) {
            targetBlock.hp--;
            digLock = ts + BREAK_MS * 0.8;
            breaking.push({ x: tx, y: ty, st: ts, type: 'hit' });
            if (graphicsSettings.particles) spawnDigParticles(tx, ty, targetBlockInfo.color || '#888', targetBlock.hp / BLOCK_TYPES.SOUL.hp);
            if (targetBlock.hp <= 0) {
            ...
            if (connectedBlocks.length >= 3) {
                 if(dy === 1 && connectedBlocks.length > 5) player.freezeUntil = ts + 1000;
  ...
  if (msState) {
      ...
      if (player.deathAnim.phase === 'lifting') {
          ...
          if (deathDuration > 1500) {
              ...
              for (let y = deathY - radius; y <= deathY + radius; y++) {
                  for (let x = deathX - radius; x <= deathX + radius; x++) {
                      if (cleared.has(`${x},${y}`)) continue;
                      const connected = findConnectedBlocks(x, y);
                      if (connected.length >= 3) {
                          connected.forEach(b => {
                              if (cleared.has(`${b.x},${b.y}`)) return;
                              const blockInfo = getBlockInfo(b.x, b.y);
                              if (graphicsSettings.particles) spawnDigParticles(b.x, b.y, blockInfo.color || '#888', 1.5);
                              setBlock(b.x, b.y, BLOCK_TYPES.EMPTY.id);
                              cleared.add(`${b.x},${b.y}`);
                          });
                      }
                  }
    ...
            } else { player.y += (direction * step); totalVerticalMovement = direction * Math.max(0, direction * totalVerticalMovement - step); }
        }
    }
    let currentPhysicalY = Math.floor(player.y);
    if (currentPhysicalY + depthOffset >= nextMilestonePhysicalY) {
        startMilestone(ts);
    }
...
    if (player.hp <= 0 || (currentBlockInfo && !currentBlockInfo.isGap && !currentBlockInfo.isHeart)){
...
function loseLife(tsNow){
    ...
    player.vy = 0;
    player.hp = HP_MAX;
    player.invincibleUntil = tsNow + 3000;
    player.deathAnim = {
        startTime: tsNow,
        phase: 'lifting',
        flashProgress: 0
...
function finishMilestone(){
    ...
    for(let y = -shardH; y < cv.height + shardH; y+=shardH) {
        for(let x = -shardW; x < cv.width + shardW; x+=shardH) {
            msShards.push({
                x: x + shardW/2, y: y + shardH/2,
                w: shardW, h: shardH,
                vx: rnd(-8, 8), vy: rnd(-15, 5),
                rot: 0, vRot: rnd(-0.2, 0.2)
            });
        }
    }
}
...
    (ctx) => { ctx.moveTo(-TILE*0.4, 0); ctx.lineTo(TILE*0.4, 0); ctx.moveTo(0, -TILE*0.4); ctx.lineTo(0, TILE*0.4); ctx.moveTo(-TILE*0.3, -TILE*0.3); ctx.lineTo(TILE*0.3, TILE*0.3); },
    (ctx) => { ctx.moveTo(-TILE*0.4, -TILE*0.1); ctx.lineTo(TILE*0.4, TILE*0.1); ctx.moveTo(-TILE*0.1, -TILE*0.4); ctx.lineTo(TILE*0.1, TILE*0.4); ctx.moveTo(TILE*0.3, -TILE*0.3); ctx.lineTo(-TILE*0.3, TILE*0.3); ctx.moveTo(-TILE*0.25, TILE*0.1); ctx.lineTo(TILE*0.1, -TILE*0.35); },
];
...
  if (!ctx) return;
  ...
  if (msState !== 'FADING_IN') {
    ...
          if (blkDef.id === BLOCK_TYPES.SOUL.id && blk.hp < blkDef.hp) { const dmg = blkDef.hp - blk.hp; const crkStg = Math.min(dmg - 1, crackPatterns.length - 1);
    ...
          if (bd.id === BLOCK_TYPES.SOUL.id && f.hp < bd.hp) { const dmg = bd.hp - f.hp; const crkStg = Math.min(dmg - 1, crackPatterns.length - 1);
            if (crkStg >= 0) { ctx.strokeStyle = 'rgba(0,0,0,0.6)'; ctx.lineWidth = Math.max(1, 2 * (TILE/60)); ctx.beginPath(); crackPatterns[crkStg](ctx); ctx.stroke(); } }
        } ctx.restore();
      });
    ...
     
      ...
      let drawPlayer = true;
      if (ts < player.invincibleUntil) { if ((ts - (player.invincibleUntil - 3000)) % 150 > 75) drawPlayer = false; }
  ...
  if (msState) {
      ...
      if (msState === 'FADING_IN') {
          ...
          ctx.fillStyle = BLOCK_TYPES.STONE.color;
          ctx.fillRect(0, 0, cv.width, cv.height);
      } else if (msState === 'BREAKING') {
          ctx.fillStyle = BLOCK_TYPES.STONE.color;
          msShards.forEach(s => {
